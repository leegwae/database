...
- 임의의 질의어가 적어도 필수적인 관계 대수 연산자들만큼의 ㅍ현력을 갖고 있으면 **관계적으로 완전(relationally complete)**하다고 한다.
...
```sql
ALTER TABLE CHAMPION ADD RELEASE_DATE DATETIME;
```
...
- `ANY`: 한 애트리뷰트가 값들의 집합에 속하는 하나 이상의 값드로가 어떤 관계를 갖는지 검사
